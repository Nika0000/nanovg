...
static float nvg__roundf(float a) { return floorf(a + 0.5f); }
...
static NVGvertex* nvg__roundJoin(NVGvertex* dst, NVGpoint* p0, NVGpoint* p1, float lw, float rw, float lu, float ru, int ncap, float fringe, float t) {
	int i, n;
	float dlx0 = p0->dy;
	float dly0 = -p0->dx;
	float dlx1 = p1->dy;
	...
	if (p1->flags & NVG_PT_LEFT) {
		...
		nvg__vset(dst, lx0, ly0, lu, 1, -1, t);
		dst++;
		nvg__vset(dst, p1->x - dlx0 * rw, p1->y - dly0 * rw, ru, 1, 1, t);
		dst++;
		...
		for (i = 0; i < n; i++) {
			...
			nvg__vset(dst, p1->x, p1->y, 0.5f, 1, 0, t);
			dst++;
			nvg__vset(dst, rx, ry, ru, 1, 1, t);
			dst++;
		}

		nvg__vset(dst, lx1, ly1, lu, 1, -1, t);
		dst++;
		nvg__vset(dst, p1->x - dlx1 * rw, p1->y - dly1 * rw, ru, 1, 1, t);
		dst++;
		...
		nvg__vset(dst, p1->x + dlx0 * rw, p1->y + dly0 * rw, lu, 1, -1, t);
		dst++;
		nvg__vset(dst, rx0, ry0, ru, 1, 1, t);
		dst++;
		...
		for (i = 0; i < n; i++) {
			...
			nvg__vset(dst, lx, ly, lu, 1, 1, t);
			dst++;
			nvg__vset(dst, p1->x, p1->y, 0.5f, 1, 0.0, t);
			dst++;
		}

		nvg__vset(dst, p1->x + dlx1 * rw, p1->y + dly1 * rw, lu, 1, -1, t);
		dst++;
		nvg__vset(dst, rx1, ry1, ru, 1, 1, t);
		dst++;
	}
	return dst;
}

static NVGvertex* nvg__bevelJoin(NVGvertex* dst, NVGpoint* p0, NVGpoint* p1, float lw, float rw, float lu, float ru, float fringe, float t) {
	...
	if (p1->flags & NVG_PT_LEFT) {
		...
		if (p1->flags & NVG_PT_BEVEL) {
			...
			nvg__vset(dst, p1->x, p1->y, 0.5f, 1, -1, t);
			dst++;
			nvg__vset(dst, p1->x - dlx0 * rw, p1->y - dly0 * rw, ru, 1, 1, t);
			dst++;
		...
		nvg__vset(dst, p1->x + dlx0 * lw, p1->y + dly0 * lw, lu, 1, 1, t);
		dst++;
		nvg__vset(dst, rx0, ry0, ru, 1, -1, t);
		dst++;
		...
		if (p1->flags & NVG_PT_BEVEL) {
			...
			nvg__vset(dst, p1->x + dlx1 * lw, p1->y + dly1 * lw, lu, 1, 1, t);
			dst++;
			nvg__vset(dst, rx1, ry1, ru, 1, -1, t);
			dst++;
		} else {
...
static NVGvertex* nvg_insertSpacer(NVGvertex* dst, NVGpoint* p, float dx, float dy, float w, float u0, float u1, float t) {
	...
	float px  = p->x;
	float py  = p->y;
	nvg__vset(dst, px + dlx * w, py + dly * w, u0, 1, -1, t);
	dst++;
	nvg__vset(dst, px - dlx * w, py - dly * w, u1, 1, 1, t);
	dst++;
	return dst;
}

static NVGvertex* nvg__buttCapStart(NVGvertex* dst, NVGpoint* p, float dx, float dy, float w, float d, float aa, float u0, float u1, float t, int dir) {
	float px  = p->x - dx * d;
	float py  = p->y - dy * d;
	float dlx = dy;
	float dly = -dx;
	nvg__vset(dst, px + dlx * w - dx * aa, py + dly * w - dy * aa, u0, 0, -1, t - dir * aa / w);
	dst++;
	nvg__vset(dst, px - dlx * w - dx * aa, py - dly * w - dy * aa, u1, 0, 1, t - dir * aa / w);
	dst++;
	nvg__vset(dst, px + dlx * w, py + dly * w, u0, 1, -1, t);
	dst++;
	nvg__vset(dst, px - dlx * w, py - dly * w, u1, 1, 1, t);
...
static NVGvertex* nvg__buttCapEnd(NVGvertex* dst, NVGpoint* p, float dx, float dy, float w, float d, float aa, float u0, float u1, float t, int dir) {
	float px  = p->x + dx * d;
	float py  = p->y + dy * d;
	float dlx = dy;
	float dly = -dx;
...
static NVGvertex* nvg__roundCapStart(NVGvertex* dst, NVGpoint* p, float dx, float dy, float w, int ncap, float aa, float u0, float u1, float t, int dir) {
	int i;
	float px  = p->x;
	float py  = p->y;
	float dlx = dy;
	...
	for (i = 0; i < ncap; i++) {
		...
		nvg__vset(dst, px - dlx * ax - dx * ay, py - dly * ax - dy * ay, u0, 1, ax / w, t - dir * ay / w);
		dst++;
		nvg__vset(dst, px, py, 0.5f, 1, 0, t);
		dst++;
	}
	nvg__vset(dst, px + dlx * w, py + dly * w, u0, 1, 1, t);
	dst++;
...
static NVGvertex* nvg__roundCapEnd(NVGvertex* dst, NVGpoint* p, float dx, float dy, float w, int ncap, float aa, float u0, float u1, float t, int dir) {
	int i;
	float px  = p->x;
	float py  = p->y;
	float dlx = dy;
	float dly = -dx;
	NVG_NOTUSED(aa);
	// s is the stroke width normalized to [-1, 1], not a pixel distance.
	nvg__vset(dst, px + dlx * w, py + dly * w, u0, 1, 1, t);
	dst++;
	nvg__vset(dst, px - dlx * w, py - dly * w, u1, 1, -1, t);
	dst++;
...
static int nvg__expandStroke(NVGcontext* ctx, float w, float fringe, int lineCap, int lineJoin, int lineStyle, float miterLimit) {
	...
	// Only dashed/dotted sample the along-path coordinate, so only they need the pattern
	// restarted at corners. Glow depends on the across-stroke coordinate alone, and the extra
	// spacer quads there just double-cover the corner geometry as a visible seam.
	const int patternStyle = (lineStyle == NVG_LINE_DASHED || lineStyle == NVG_LINE_DOTTED);
	...
	for (i = 0; i < cache->npaths; i++) {
		...
		if (patternStyle && loop) {
			float plen = 0.0f;
			for (j = 0; j < path->count; ++j)
				plen += pts[j].len;
			float periods = plen * invStrokeWidth / 8.0f;
			if (periods > 1.0f) pathScale = nvg__roundf(periods) / periods;
		}
		...
		for (j = s; j < e; ++j) {
			if (lineStyle > 1) {
				...
				// Only break the pattern coordinate at real corners. Spacers are extruded along the
				// segment normal, while the strip uses the point's miter normal (dmx/dmy); on smooth
				// tessellated curves the two disagree and the spacer quads stick out as spikes.
				if (patternStyle && (p0->flags & NVG_PT_CORNER)) dst = nvg_insertSpacer(dst, p0, dx, dy, w, u0, u1, t);
				t += dir * dt * tScale;
				if (patternStyle && (p1->flags & NVG_PT_CORNER)) dst = nvg_insertSpacer(dst, p1, dx, dy, w, u0, u1, t);
			}
			if ((p1->flags & (NVG_PT_BEVEL | NVG_PR_INNERBEVEL)) != 0) {
				if (lineJoin == NVG_ROUND) {
				...
				nvg__vset(dst, p1->x + (p1->dmx * w), p1->y + (p1->dmy * w), u0, 1, -1, t);
				dst++;
				nvg__vset(dst, p1->x - (p1->dmx * w), p1->y - (p1->dmy * w), u1, 1, 1, t);
				dst++;
			}
			p0 = p1++;
		}
			...
			for (j = 0; j < path->count; ++j) {
				if (p1->flags & NVG_PT_BEVEL) {
					...
					if (p1->flags & NVG_PT_LEFT) {
						...
						nvg__vset(dst, lx, ly, 0.5f, 1, 0, 0);
						dst++;
					} else {
						...
						nvg__vset(dst, lx0, ly0, 0.5f, 1, 0, 0);
						dst++;
						nvg__vset(dst, lx1, ly1, 0.5f, 1, 0, 0);
						dst++;
					}
				} else {
					nvg__vset(dst, p1->x + (p1->dmx * woff), p1->y + (p1->dmy * woff), 0.5f, 1, 0, 0);
					dst++;
				}
				p0 = p1++;
			}
		...
		if (fringe) {
			...
			for (j = 0; j < path->count; ++j) {
				if ((p1->flags & (NVG_PT_BEVEL | NVG_PR_INNERBEVEL)) != 0) {
					...
					nvg__vset(dst, p1->x + (p1->dmx * lw), p1->y + (p1->dmy * lw), lu, 1, 0, 0);
					dst++;
					nvg__vset(dst, p1->x - (p1->dmx * rw), p1->y - (p1->dmy * rw), ru, 1, 0, 0);
					dst++;
				}
				p0 = p1++;
			}
			...
			// Loop it
			nvg__vset(dst, verts[0].x, verts[0].y, lu, 1, 0, 0);
			dst++;
			nvg__vset(dst, verts[1].x, verts[1].y, ru, 1, 0, 0);
			dst++;
...
void nvgStroke(NVGcontext* ctx) {
	...
	ctx->params.renderStroke(ctx->params.userPtr, &strokePaint, state->compositeOperation, &state->scissor, ctx->fringeWidth, strokeWidth, state->lineStyle, ctx->cache->paths, ctx->cache->npaths);
...
float nvgText(NVGcontext* ctx, float x, float y, const char* string, const char* end) {
	...
	while (fonsTextIterNext(ctx->fs, &iter, &q)) {
		...
		if (nverts + 6 <= cverts) {
			...
			nverts++;
			nvg__vset(&verts[nverts], c[4], c[5], q.s1, q.t1, 0, 0);
			nverts++;
			nvg__vset(&verts[nverts], c[2], c[3], q.s1, q.t0, 0, 0);
			nverts++;
			nvg__vset(&verts[nverts], c[0], c[1], q.s0, q.t0, 0, 0);
			nverts++;
			nvg__vset(&verts[nverts], c[6], c[7], q.s0, q.t1, 0, 0);
			nverts++;
			nvg__vset(&verts[nverts], c[4], c[5], q.s1, q.t1, 0, 0);
			nverts++;
		}
